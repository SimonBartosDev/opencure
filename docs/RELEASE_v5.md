...
- **Silver-standard triangulation currently unreachable.** On the live 8-disease sample, 0/80 candidates hit ≥3 axes. Docking axis is scaffolded with a `not_wired` marker on every candidate (with a ChEMBL-bioactivity proxy that fires when drug-disease target geometry exists in OT — 0/90 backfill hits on neglected-tropical diseases due to thin OT coverage). True AutoDock Vina integration is v6 work.
